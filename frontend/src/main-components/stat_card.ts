...
function winLossBarGraph(win_count: number, loss_count: number) {
    ...
    <div class="flex mx-auto mt-4 h-6 overflow-hidden">
        <div class="flex items-center w-11/12 justify-center text-xs font-bold bg-green-700 rounded-sm" style="width: ${win_percent}%;">${(win_count > 0) ? win_count : ""}</div>
        <div class="flex items-center w-11/12 justify-center text-xs font-bold bg-red-700 rounded-sm" style="width: ${100 - win_percent}%;">${(loss_count > 0) ? loss_count : ""}</div>
    </div>
    `
}
